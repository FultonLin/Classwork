...
public class MovingComponent extends Component implements Runnable {
	...
	public void update(Graphics2D g) {
		...
		long difference = currentTime - moveTime;//this calculates time since last move
		if(difference >= REFRESH_RATE){
			//an update is happening so update movTime
			moveTime = currentTime;
			//calculate new position
			posx += vx*(double)difference/REFRESH_RATE;			
			posy += vy*(double)difference/REFRESH_RATE;
			//set only the location on the screen NOT the actual position
			super.setX((int)posx);
			super.setY((int)posy);
		}
		drawImage(g);
